...
/*
 * arguments: fastinf outfile, dataset dir, scene names
 */
int main(int argc, char* argv[])
{
	...
	unsigned int _ = 1;
	const fs::path fastinfOutpath(argv[_++]), datasetDir(argv[_++]);
	vector<string> sceneNames(argc - _);
	for(unsigned int i = 0; _ < argc; _++, i++) sceneNames[i] = argv[_];
	ASSERT_ALWAYS(sceneNames.size() == 2);
	const unsigned int numScenes = sceneNames.size();

	/********************************************************************************************************
	 * read datasets
	 */

	vector<boost::shared_ptr<sceneInfo>> scenes(numScenes);
	for(unsigned int i = 0; i < numScenes; i++)
	{
		sceneInitOptions opts;
		opts.computePrincipalCurvatures = false;
		opts.readSpinImgs = false;
		scenes[i] = boost::make_shared<sceneInfo>();
		*scenes[i] = std::move(initializeScene(datasetDir, sceneNames[i], opts));
	}
#if 0
	...
	*/
...
#endif
	ifstream infile(fastinfOutpath.string());
	ASSERT_ALWAYS(infile);
	boost::regex varCountRex("The first (\\d+) marginals");
	string line;
	boost::smatch match;
	while(getline(infile, line) && !boost::regex_search(line, match, varCountRex)) ;
	const unsigned int numVars = lexical_cast<unsigned int>(match[1]);
	...
	for(unsigned int j = 0; j < numVars; j++)
	{
		...
		vector<double> probs;
		double p;
		while(instr >> p) probs.push_back(p);
		cout << " " << probs.size() << " probs" << endl;
	}
	infile.close();
#if 0
	/*

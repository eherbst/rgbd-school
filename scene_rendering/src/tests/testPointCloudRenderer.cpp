/*
 * testPointCloudRenderer: test a pointCloudRenderer with opengl and glx
 */
...
int main(int argc, char **argv)
{
	...
	pointCloudRenderer renderer(cloud, 1, camParams);
	scorer.acquire();
	scorer.setRenderFunc([&renderer](const rgbd::eigen::Affine3f& xform){renderer.render(xform);});
	cv::Mat cvImg(camParams.yRes, camParams.xRes, CV_8UC3);
	for(unsigned int i = 0; i < rots.size(); i++)
	{
		...
		cout << score << endl;
		cv::imwrite((boost::format("cloud%1%.png") % i).str().c_str(), cvImg);
	}
	scorer.release();
